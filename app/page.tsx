        ...
        <SidebarBody className="justify-between gap-10 glass-strong rounded-2xl border border-white/10 shadow-lg shadow-black/20 backdrop-blur-xl">
          <div className="flex flex-1 flex-col overflow-x-hidden overflow-y-auto">
            ...
            <div className="mt-8 flex flex-col gap-2">
              {links.map((link) => (
                <div key={`nav-${link.label}`} onClick={() => scrollToSection(link.href)} className="cursor-pointer">
...
const Logo = memo(() => (
  ...
  >
    <img
      ...
      loading="eager"
    />
    <span className="font-bold whitespace-pre text-gradient text-base sm:text-lg">CODE VIVEKS</span>
  </motion.div>
))
...
const LogoIcon = memo(() => (
  <motion.div className="relative z-20 flex items-center justify-center py-1" whileHover={{ scale: 1.1 }}>
    <img
      src={logo.src || "/placeholder.svg"}
...
const ThemeToggle = memo(({ open }: { open: boolean }) => {
  ...
  return (
    ...
    >
      ...
      {open && <span className="text-sm font-semibold">{theme === "dark" ? "Light" : "Dark"}</span>}
    </motion.button>
  )
})
